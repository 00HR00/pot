...
def main(args):
    ...
    if args.dataset == 'CIFAR10':
        if args.model == 'ConvNet':
            ...
            buffer_s_data_dir = os.path.join(args.intermediate_result_save_path, "attacker_2_cifar10_ConvNet_b")
            ...
            buffer_s_data_dir = os.path.join(args.intermediate_result_save_path, "attacker_2_cifar10_ConvNetD2_b")
            target_M_network = get_target_M_network(honest_length,target_path).to(args.device)
            initial_M_network = get_initial_M_network(0,target_path).to(args.device)

        elif args.model == 'ConvNetD4':
            target_path = os.path.join(args.intermediate_result_save_path, "honest_cifar10_ConvNetD4_t")
            buffer_s_data_dir = os.path.join(args.intermediate_result_save_path, "attacker_2_cifar10_ConvNetD4_b")
            target_M_network = get_target_M_network(honest_length,target_path).to(args.device)
            initial_M_network = get_initial_M_network(0,target_path).to(args.device)

        elif args.model == 'ResNet18':
            target_path = os.path.join(args.intermediate_result_save_path, "honest_cifar10_ResNet18_t")
            buffer_s_data_dir = os.path.join(args.intermediate_result_save_path, "attacker_2_cifar10_ResNet18_b")
            target_M_network = get_target_M_network(honest_length,target_path).to(args.device)
            initial_M_network = get_initial_M_network(0,target_path).to(args.device)
            
        elif args.model == 'ResNet34':
            target_path = os.path.join(args.intermediate_result_save_path, "honest_cifar10_ResNet34_t")
            buffer_s_data_dir = os.path.join(args.intermediate_result_save_path, "attacker_2_cifar10_ResNet34_b")
            target_M_network = get_target_M_network(honest_length,target_path).to(args.device)
            initial_M_network = get_initial_M_network(0,target_path).to(args.device)
        ...
        if args.model == 'ConvNet':
            ...
            buffer_s_data_dir = os.path.join(args.intermediate_result_save_path, "attacker_2_cifar100_ConvNet_b")
            target_M_network = get_target_M_network(honest_length,target_path).to(args.device)
            initial_M_network = get_initial_M_network(0,target_path).to(args.device)

        elif args.model == 'ConvNetD2':
            target_path = os.path.join(args.intermediate_result_save_path, "honest_cifar100_ConvNetD2_t")
            buffer_s_data_dir = os.path.join(args.intermediate_result_save_path, "attacker_2_cifar100_ConvNetD2_b")
            target_M_network = get_target_M_network(honest_length,target_path).to(args.device)
            initial_M_network = get_initial_M_network(0,target_path).to(args.device)

        elif args.model == 'ConvNetD4':
            target_path = os.path.join(args.intermediate_result_save_path, "honest_cifar100_ConvNetD4_t")
            buffer_s_data_dir = os.path.join(args.intermediate_result_save_path, "attacker_2_cifar100_ConvNetD4_b")
            target_M_network = get_target_M_network(honest_length,target_path).to(args.device)
            initial_M_network = get_initial_M_network(0,target_path).to(args.device)
            
        elif args.model == 'ResNet18':
            target_path = os.path.join(args.intermediate_result_save_path, "honest_cifar100_ResNet18_t")
            buffer_s_data_dir = os.path.join(args.intermediate_result_save_path, "attacker_2_cifar100_ResNet18_b")
            target_M_network = get_target_M_network(honest_length,target_path).to(args.device)
            initial_M_network = get_initial_M_network(0,target_path).to(args.device)
            
        elif args.model == 'ResNet34':
            target_path = os.path.join(args.intermediate_result_save_path, "honest_cifar100_ResNet34_t")
            buffer_s_data_dir = os.path.join(args.intermediate_result_save_path, "attacker_2_cifar100_ResNet34_b")
            target_M_network = get_target_M_network(honest_length,target_path).to(args.device)
            initial_M_network = get_initial_M_network(0,target_path).to(args.device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_length', type=float, default=150, help='trajectory_length')
